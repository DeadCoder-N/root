import { X, Terminal, AlertTriangle, Download, Loader2 } from 'lucide-react';
import { useState } from 'react';

interface CommandInjectionModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function CommandInjectionModal({ isOpen, onClose }: CommandInjectionModalProps) {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5017/api/analyze', {
        ...
        body: JSON.stringify({ url: url.trim(), parameter: parameter.trim() || 'cmd' }),
      });
      ...
      setError(err.message || 'Failed to scan for command injection');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5017/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `command-injection-report-${Date.now()}.pdf`;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-red-500 to-rose-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Terminal className="w-8 h-8 text-white" />
            <div>
              <h2 className="text-2xl font-bold text-white">Command Injection Scanner</h2>
              <p className="text-white/80 text-sm">Scan for OS command injection vulnerabilities</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="https://example.com/api/execute"
                className="w-full px-4 py-2 bg-secondary border border-white/10 rounded-lg focus:outline-none focus:border-red-500/50"
                disabled={loading}
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">Parameter Name (optional)</label>
              <input
                type="text"
                value={parameter}
            ...
            <button
              onClick={handleScan}
              disabled={loading}
              className="w-full bg-gradient-to-r from-red-500 to-rose-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                ...
                <div className="space-y-3">
                  ...
                  {results.vulnerabilities.map((vuln: any, index: number) => (
                    <div key={index} className="p-4 bg-secondary border border-white/10 rounded-lg space-y-2">
                      ...
                      {vuln.fix_prompt && (
                        <div className="mt-2 p-3 bg-primary border border-red-500/30 rounded">
                          <div className="text-xs text-red-500 font-medium mb-1">Fix Recommendation:</div>
          ...
          {!results && !loading && !error && (
            <div className="text-center py-12 space-y-4">
              <Terminal className="w-16 h-16 text-text-muted mx-auto opacity-50" />
              <div>
                <h3 className="text-lg font-semibold mb-2">How to Use</h3>
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the target URL with command execution</p>
                  <p>• Optionally specify parameter name</p>
                  <p>• Tests OS command injection, path traversal, LFI</p>
                  <p>• Get comprehensive report with fix recommendations</p>
                </div>
              </div>
            </div>
          )}
