...
interface SSLScannerModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function SSLScannerModal({ isOpen, onClose }: SSLScannerModalProps) {
  ...
  const handleScan = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5010/api/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ hostname: hostname.trim(), port: parseInt(port) }),
      });
      ...
      setError(err.message || 'Failed to scan SSL/TLS configuration');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5010/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `ssl-scan-report-${Date.now()}.pdf`;
      ...
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-indigo-500 to-purple-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">SSL/TLS Scanner</h2>
              <p className="text-white/80 text-sm">Scan SSL/TLS configuration and certificates</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 bg-secondary border border-white/10 rounded-lg focus:outline-none focus:border-indigo-500/50"
                disabled={loading}
              />
            </div>
            ...
            <button
              onClick={handleScan}
              disabled={loading}
              className="w-full bg-gradient-to-r from-indigo-500 to-purple-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                ...
                <div className="space-y-3">
                  ...
                  {results.vulnerabilities.map((vuln: any, index: number) => (
                    <div key={index} className="p-4 bg-secondary border border-white/10 rounded-lg space-y-2">
                      ...
                      {vuln.fix_prompt && (
                        <div className="mt-2 p-3 bg-primary border border-indigo-500/30 rounded">
                          <div className="text-xs text-indigo-500 font-medium mb-1">Fix Recommendation:</div>
            ...
            <div className="text-center py-12 space-y-4">
              ...
              <div>
                ...
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the hostname (e.g., example.com)</p>
                  <p>• Specify port (default: 443)</p>
                  <p>• Tests certificate validation, weak protocols, cipher suites</p>
                  <p>• Get comprehensive SSL/TLS security report</p>
                </div>
              </div>
            </div>
          )}
