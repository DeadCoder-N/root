"""DNS Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"dns_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="DNS Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("DNS SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive DNS Configuration & Security Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['DNS Security', self._check_dns_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected DNS records and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _group_vulnerabilities(self, vulnerabilities):
        ...
        for vuln in vulnerabilities:
            ...
            groups[vuln_type]['locations'].append({'location': vuln.get('domain', vuln.get('record', 'N/A')), 'evidence': vuln.get('evidence', 'N/A')})
    ...
    def _format_vulnerability_group(self, group):
        ...
        elements.append(Paragraph(f"<b>🌐 Affected Records ({group['count']}):</b>", self.body_style))
        for i, loc in enumerate(group['locations'][:10]):
            elements.append(Paragraph(f"  • {loc['location']}", self.body_style))
        if group['count'] > 10:
            elements.append(Paragraph(f"  ... and {group['count'] - 10} more records", self.body_style))
        if group['fix_prompt']:
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'DNS Zone Transfer Enabled': 'Zone transfers expose all DNS records including internal hostnames, IPs, and infrastructure details. Attackers use this for reconnaissance.',
            'Missing SPF Record': 'Without SPF, attackers can spoof emails from your domain, damaging reputation and enabling phishing attacks against customers.',
            'Missing DMARC': 'DMARC prevents email spoofing and phishing. Without it, attackers can send fake emails appearing to come from your domain.',
            'DNSSEC Not Enabled': 'Without DNSSEC, DNS responses can be spoofed, redirecting users to malicious sites in man-in-the-middle attacks.',
            'Wildcard DNS Record': 'Wildcard records can be exploited for subdomain takeover attacks, allowing attackers to host malicious content on your domain.',
            'Missing CAA Record': 'CAA records prevent unauthorized certificate issuance. Without them, attackers can obtain valid SSL certificates for your domain.',
        }
        return explanations.get(vuln_type, 'This DNS vulnerability exposes infrastructure information or enables spoofing attacks.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'DNS Zone Transfer Enabled': '"Disable zone transfers. BIND: in named.conf add: allow-transfer { none; }; or allow-transfer { trusted-servers; };. Restart: systemctl restart named"',
            'Missing SPF Record': '"Add SPF record to DNS. Example: v=spf1 ip4:YOUR_IP include:_spf.google.com ~all. Add TXT record: dig TXT example.com to verify. Use SPF wizard: spfwizard.net"',
            'Missing DMARC': '"Add DMARC record: _dmarc.example.com TXT v=DMARC1; p=quarantine; rua=mailto:dmarc@example.com. Start with p=none, monitor reports, then enforce with p=quarantine or p=reject"',
            'DNSSEC Not Enabled': '"Enable DNSSEC at registrar and DNS provider. Generate keys: dnssec-keygen -a RSASHA256 -b 2048 example.com. Sign zone: dnssec-signzone. Add DS records to registrar"',
            'Wildcard DNS Record': '"Remove wildcard DNS: delete *.example.com A record. Create specific subdomains only. If needed, use wildcard with strict validation and monitoring"',
            'Missing CAA Record': '"Add CAA record to restrict certificate issuance: example.com CAA 0 issue letsencrypt.org. Add: example.com CAA 0 issuewild letsencrypt.org for wildcards"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by updating DNS records with proper security configurations"')
    ...
    def _check_dns_compliance(self, severity):
