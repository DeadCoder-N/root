import { X, Shield, AlertTriangle, CheckCircle, Download, Loader2 } from 'lucide-react';
...
export default function AuthenticationModal({ isOpen, onClose }: AuthenticationModalProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5006/api/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: url.trim(), username: username.trim() || 'admin' }),
      });
      ...
      setError(err.message || 'Failed to analyze authentication');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5006/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `authentication-report-${Date.now()}.pdf`;
      ...
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-red-500 to-orange-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Authentication Analyzer</h2>
              <p className="text-white/80 text-sm">Test authentication security mechanisms</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Test Username (optional)</label>
              <input
                type="text"
                value={username}
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-red-500 to-orange-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          {!results && !loading && !error && (
            <div className="text-center py-12 space-y-4">
              <Shield className="w-16 h-16 text-text-muted mx-auto opacity-50" />
              <div>
                <h3 className="text-lg font-semibold mb-2">How to Use</h3>
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the login page URL</p>
                  <p>• Optionally specify a test username</p>
                  <p>• Tests brute force protection, password policy, session management, 2FA</p>
                  <p>• Get detailed vulnerability report with fix recommendations</p>
                </div>
              </div>
            </div>
          )}
