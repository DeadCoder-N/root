"""CSRF Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"csrf_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="CSRF Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("CSRF SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive Cross-Site Request Forgery Protection Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['CSRF Protection', self._check_csrf_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected forms/endpoints and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _group_vulnerabilities(self, vulnerabilities):
        ...
        for vuln in vulnerabilities:
            ...
            groups[vuln_type]['locations'].append({'location': vuln.get('form', vuln.get('url', 'N/A')), 'evidence': vuln.get('evidence', 'N/A')})
    ...
    def _format_vulnerability_group(self, group):
        ...
        elements.append(Paragraph(f"<b>📋 Affected Forms ({group['count']}):</b>", self.body_style))
        for i, loc in enumerate(group['locations'][:10]):
            elements.append(Paragraph(f"  • {loc['location']}", self.body_style))
        if group['count'] > 10:
            elements.append(Paragraph(f"  ... and {group['count'] - 10} more forms", self.body_style))
        if group['fix_prompt']:
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Missing CSRF Token': 'Without CSRF tokens, attackers can trick users into performing unwanted actions (money transfers, password changes) by embedding malicious requests in emails or websites.',
            'Weak CSRF Token': 'Predictable or reused tokens can be guessed by attackers, allowing them to forge valid requests and bypass CSRF protection.',
            'CSRF Token in GET Request': 'Tokens in URLs are logged in browser history, proxy logs, and referrer headers, allowing attackers to steal them and forge requests.',
            'Missing SameSite Cookie': 'Without SameSite attribute, cookies are sent in cross-site requests, enabling CSRF attacks even with token validation if tokens are in cookies.',
            'CSRF Token Not Validated': 'If server doesn\'t validate tokens or accepts requests without them, CSRF protection is ineffective and attackers can forge requests.',
            'Token Reuse Across Sessions': 'Tokens that don\'t expire or are reused allow attackers to use stolen tokens indefinitely, even after user logs out.',
        }
        return explanations.get(vuln_type, 'This CSRF vulnerability allows attackers to perform unauthorized actions on behalf of authenticated users.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Missing CSRF Token': '"Implement CSRF tokens. Flask: from flask_wtf.csrf import CSRFProtect; csrf = CSRFProtect(app). Django: add {% csrf_token %} in forms. Express: npm install csurf; app.use(csrf())"',
            'Weak CSRF Token': '"Generate strong tokens: import secrets; token = secrets.token_urlsafe(32). Store in session, validate on POST: if request.form[\'csrf_token\'] != session[\'csrf_token\']: abort(403)"',
            'CSRF Token in GET Request': '"Move tokens to POST body or headers. Never use GET for state-changing operations. Use POST/PUT/DELETE with token in body: <input type=\'hidden\' name=\'csrf_token\' value=\'{{token}}\'>"',
            'Missing SameSite Cookie': '"Add SameSite to cookies: Set-Cookie: sessionid=xxx; SameSite=Strict; HttpOnly; Secure. Flask: SESSION_COOKIE_SAMESITE = \'Strict\'. Django: SESSION_COOKIE_SAMESITE = \'Strict\'"',
            'CSRF Token Not Validated': '"Validate tokens on server: if not validate_csrf_token(request): abort(403). Check token exists, matches session, and hasn\'t expired. Reject requests without valid tokens"',
            'Token Reuse Across Sessions': '"Regenerate tokens per session: on login: session[\'csrf_token\'] = generate_token(). Expire tokens: store timestamp, validate age < 1 hour. Invalidate on logout"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by implementing proper CSRF token generation, validation, and SameSite cookie attributes"')
    ...
    def _check_csrf_compliance(self, severity):
