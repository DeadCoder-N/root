"""Port Scanner Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"port_scan_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="Port Scan Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("PORT SCAN SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive Network Port & Service Exposure Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['Network Security', self._check_network_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected ports and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _group_vulnerabilities(self, vulnerabilities):
        ...
        for vuln in vulnerabilities:
            ...
            groups[vuln_type]['locations'].append({'location': f"Port {vuln.get('port', 'N/A')}", 'evidence': vuln.get('evidence', 'N/A')})
    ...
    def _format_vulnerability_group(self, group):
        ...
        elements.append(Paragraph(f"<b>🔌 Affected Ports ({group['count']}):</b>", self.body_style))
        for i, loc in enumerate(group['locations'][:10]):
            elements.append(Paragraph(f"  • {loc['location']}", self.body_style))
        if group['count'] > 10:
            elements.append(Paragraph(f"  ... and {group['count'] - 10} more ports", self.body_style))
        if group['fix_prompt']:
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Exposed Database Port': 'Database ports exposed to internet allow attackers to attempt brute force attacks, exploit vulnerabilities, or steal data directly without going through application.',
            'Exposed SSH Port': 'SSH exposed to internet is prime target for brute force attacks. Attackers scan for port 22 and try common credentials to gain server access.',
            'Exposed RDP Port': 'Remote Desktop exposed to internet allows attackers to brute force Windows credentials and gain full GUI access to server.',
            'Exposed Admin Port': 'Admin interfaces on public ports allow attackers to discover and exploit management panels, often with default credentials.',
            'Unnecessary Open Ports': 'Each open port is potential attack vector. Unused services should be disabled to reduce attack surface.',
            'Exposed FTP Port': 'FTP transmits credentials in plaintext and has known vulnerabilities. Exposed FTP allows credential theft and unauthorized file access.',
        }
        return explanations.get(vuln_type, 'This exposed port increases attack surface and provides entry point for attackers.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Exposed Database Port': '"Block database port from internet. Firewall: ufw deny 3306/tcp (MySQL) or ufw deny 5432/tcp (PostgreSQL). Allow only from application server: ufw allow from 10.0.1.5 to any port 3306"',
            'Exposed SSH Port': '"Restrict SSH to specific IPs. Firewall: ufw allow from YOUR_IP to any port 22. Or change port: edit /etc/ssh/sshd_config, set Port 2222, restart: systemctl restart sshd"',
            'Exposed RDP Port': '"Block RDP from internet. Windows Firewall: New-NetFirewallRule -DisplayName \'Block RDP\' -Direction Inbound -LocalPort 3389 -Protocol TCP -Action Block. Use VPN for remote access"',
            'Exposed Admin Port': '"Close admin ports. Stop service: systemctl stop admin-service. Firewall: ufw deny 8080/tcp. Access via SSH tunnel: ssh -L 8080:localhost:8080 user@server"',
            'Unnecessary Open Ports': '"Close unused ports. List services: netstat -tulpn. Stop service: systemctl stop service-name. Disable: systemctl disable service-name. Block: ufw deny PORT/tcp"',
            'Exposed FTP Port': '"Disable FTP, use SFTP instead. Stop FTP: systemctl stop vsftpd. Enable SFTP: edit /etc/ssh/sshd_config, add Subsystem sftp /usr/lib/openssh/sftp-server"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by implementing firewall rules and restricting access to trusted IPs only"')
    ...
    def _check_network_compliance(self, severity):
