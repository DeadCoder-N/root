...
def add_fix_prompts(vulnerabilities):
    ...
    for vuln in vulnerabilities:
        ...
        if 'Sensitive File' in vuln_type:
            vuln['fix_prompt'] = "Remove sensitive files from web root. Add to .gitignore. Use robots.txt to block crawlers. Configure web server to deny access."
        elif 'Directory Listing' in vuln_type:
            vuln['fix_prompt'] = "Disable directory listing. Apache: Options -Indexes. Nginx: autoindex off. Add index.html to all directories."
        elif 'Backup' in vuln_type:
            vuln['fix_prompt'] = "Remove backup files from production. Store backups outside web root. Use .htaccess to block access to backup extensions."
        else:
            vuln['fix_prompt'] = f"Review and fix {vuln_type}. Check OWASP Information Disclosure guidelines."
    
    return vulnerabilities

@app.route('/health', methods=['GET'])
def health():
    return jsonify({"status": "healthy", "port": 5009}), 200

@app.route('/api/analyze', methods=['POST'])
def analyze():
    try:
        data = request.get_json()
        crawler = WebCrawler(data['target_url'])
        results = crawler.crawl(max_pages=data.get('max_pages', 50))
        ...
        report_id = f"crawler_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}"
...
def download(report_id):
    try:
        ...
        gen = UniversalReportGenerator(results, "Web Crawler")
        pdf = gen.generate()
