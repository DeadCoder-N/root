"""Business Logic Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"business_logic_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="Business Logic Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("BUSINESS LOGIC SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive Business Logic Flaw & Workflow Security Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['Logic Security', self._check_logic_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected business logic flows and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _group_vulnerabilities(self, vulnerabilities):
        ...
        for vuln in vulnerabilities:
            ...
            groups[vuln_type]['locations'].append({'location': vuln.get('endpoint', vuln.get('url', 'N/A')), 'evidence': vuln.get('evidence', 'N/A')})
    ...
    def _format_vulnerability_group(self, group):
        ...
        elements.append(Paragraph(f"<b>💼 Affected Workflows ({group['count']}):</b>", self.body_style))
        for i, loc in enumerate(group['locations'][:10]):
            elements.append(Paragraph(f"  • {loc['location']}", self.body_style))
        if group['count'] > 10:
            elements.append(Paragraph(f"  ... and {group['count'] - 10} more workflows", self.body_style))
        if group['fix_prompt']:
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Price Manipulation': 'Attackers can modify prices in requests to purchase items for $0.01 or negative amounts, causing direct financial loss to business.',
            'Quantity Manipulation': 'Negative quantities can be exploited to receive refunds without returning items, or bypass inventory limits to purchase more than available stock.',
            'Race Condition': 'Concurrent requests can exploit timing windows to redeem coupons multiple times, withdraw more money than available, or bypass rate limits.',
            'Workflow Bypass': 'Attackers can skip payment steps, approval processes, or verification checks by manipulating request sequences or parameters.',
            'Insufficient Authorization': 'Users can access or modify resources belonging to other users by changing IDs in requests, leading to data breaches and unauthorized actions.',
            'Rate Limit Bypass': 'Missing or weak rate limiting allows attackers to abuse expensive operations, scrape data, or perform brute force attacks without restriction.',
        }
        return explanations.get(vuln_type, 'This business logic flaw allows attackers to abuse application workflows and cause financial or operational damage.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Price Manipulation': '"Validate prices server-side: stored_price = get_price_from_db(product_id); if request.price != stored_price: reject(). Never trust client-sent prices. Recalculate totals on server"',
            'Quantity Manipulation': '"Validate quantity: if quantity < 1 or quantity > MAX_QUANTITY: reject(). Check inventory: if quantity > available_stock: reject(). Use database constraints: CHECK (quantity > 0)"',
            'Race Condition': '"Use database transactions with locks: BEGIN TRANSACTION; SELECT balance FOR UPDATE; if balance >= amount: UPDATE balance; COMMIT; else: ROLLBACK. Implement idempotency keys for operations"',
            'Workflow Bypass': '"Enforce workflow state: store state in session/db. Validate: if current_state != expected_state: reject(). Use state machine pattern. Check prerequisites before each step"',
            'Insufficient Authorization': '"Implement authorization checks: user_id = get_current_user(); resource = get_resource(id); if resource.owner_id != user_id: abort(403). Check ownership on every request"',
            'Rate Limit Bypass': '"Implement rate limiting: from flask_limiter import Limiter; limiter = Limiter(app, key_func=get_remote_address); @limiter.limit(\'10 per minute\'). Use Redis for distributed rate limiting"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by implementing server-side validation, authorization checks, and proper workflow state management"')
    ...
    def _check_logic_compliance(self, severity):
