import { X, FileCode, AlertTriangle, Download, Loader2 } from 'lucide-react';
import { useState } from 'react';

interface XXEModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function XXEModal({ isOpen, onClose }: XXEModalProps) {
  ...
  const handleScan = async () => {
    if (!url.trim()) {
      setError('Please enter an XML endpoint URL');
      return;
    }
    ...
    try {
      ...
      setError(err.message || 'Failed to scan for XXE vulnerabilities');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5015/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `xxe-scan-report-${Date.now()}.pdf`;
      ...
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-purple-500 to-indigo-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">XXE Scanner</h2>
              <p className="text-white/80 text-sm">Scan for XML External Entity vulnerabilities</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">XML Endpoint URL *</label>
              <input
                type="url"
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                placeholder="https://example.com/api/xml"
                className="w-full px-4 py-2 bg-secondary border border-white/10 rounded-lg focus:outline-none focus:border-purple-500/50"
                disabled={loading}
              />
            </div>
            ...
            <button
              onClick={handleScan}
              disabled={loading}
              className="w-full bg-gradient-to-r from-purple-500 to-indigo-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                ...
                <div className="space-y-3">
                  ...
                  {results.vulnerabilities.map((vuln: any, index: number) => (
                    <div key={index} className="p-4 bg-secondary border border-white/10 rounded-lg space-y-2">
                      ...
                      {vuln.fix_prompt && (
                        <div className="mt-2 p-3 bg-primary border border-purple-500/30 rounded">
                          <div className="text-xs text-purple-500 font-medium mb-1">Fix Recommendation:</div>
          ...
          {!results && !loading && !error && (
            <div className="text-center py-12 space-y-4">
              <FileCode className="w-16 h-16 text-text-muted mx-auto opacity-50" />
              <div>
                <h3 className="text-lg font-semibold mb-2">How to Use</h3>
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the XML processing endpoint URL</p>
                  <p>• Tests for file reading via external entities</p>
                  <p>• Checks XML bomb (billion laughs attack)</p>
                  <p>• Detects SSRF via XXE vulnerabilities</p>
                </div>
              </div>
            </div>
          )}
