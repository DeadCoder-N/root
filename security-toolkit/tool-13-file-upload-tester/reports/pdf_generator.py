"""File Upload Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"file_upload_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="File Upload Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("FILE UPLOAD SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive File Upload Validation & Security Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['Upload Security', self._check_upload_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected upload endpoints and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Unrestricted File Upload': 'Attackers can upload malicious files (PHP shells, malware) and execute them on server, gaining complete control of the application and data.',
            'No File Type Validation': 'Without validation, attackers upload executable files disguised as images, leading to remote code execution and server compromise.',
            'Missing File Size Limit': 'Unlimited file sizes enable denial of service attacks by filling disk space, crashing server, or consuming bandwidth.',
            'Executable File Upload': 'Uploaded executables (.php, .jsp, .asp) can be accessed directly via browser, allowing attackers to run arbitrary code on server.',
            'Path Traversal in Upload': 'Attackers can manipulate filenames to write files outside upload directory, overwriting critical system files or placing backdoors.',
            'No Content-Type Validation': 'Relying only on Content-Type header is insecure as attackers can fake it. Files must be validated by actual content/magic bytes.',
        }
        return explanations.get(vuln_type, 'This file upload vulnerability allows attackers to upload malicious files and potentially execute code on the server.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Unrestricted File Upload': '"Implement whitelist validation: allowed_ext = [\'.jpg\', \'.png\', \'.pdf\']; if ext not in allowed_ext: reject(). Validate MIME type and magic bytes. Store uploads outside web root"',
            'No File Type Validation': '"Add file type validation: import magic; mime = magic.from_buffer(file.read(1024), mime=True); if mime not in [\'image/jpeg\', \'image/png\']: reject(). Check extension AND content"',
            'Missing File Size Limit': '"Set file size limit: MAX_SIZE = 5 * 1024 * 1024 (5MB); if file.size > MAX_SIZE: reject(). In Flask: app.config[\'MAX_CONTENT_LENGTH\'] = 5 * 1024 * 1024"',
            'Executable File Upload': '"Block executable extensions: blocked = [\'.php\', \'.jsp\', \'.asp\', \'.sh\']; if ext in blocked: reject(). Store uploads outside web root. Set execute permissions: chmod 644"',
            'Path Traversal in Upload': '"Sanitize filename: import os; filename = os.path.basename(file.filename); filename = re.sub(r\'[^a-zA-Z0-9._-]\', \'\', filename). Never use user input directly in file path"',
            'No Content-Type Validation': '"Validate file content: import imghdr; if imghdr.what(file) not in [\'jpeg\', \'png\']: reject(). Use python-magic for MIME detection. Don\'t trust Content-Type header"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by implementing strict file validation, size limits, and storing uploads outside web root"')
    ...
    def _check_upload_compliance(self, severity):
