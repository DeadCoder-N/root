"""Session Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"session_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="Session Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("SESSION SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive Session Management & Cookie Security Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['Session Security', self._check_session_compliance(severity), ''],
        ]
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Session Fixation': 'Attackers can force users to use attacker-controlled session IDs, allowing session hijacking after login. Session IDs must be regenerated on authentication.',
            'Missing HttpOnly Flag': 'Without HttpOnly flag, JavaScript can access session cookies, making them vulnerable to XSS attacks that steal session tokens.',
            'Missing Secure Flag': 'Cookies without Secure flag are transmitted over HTTP, allowing network attackers to intercept session tokens via man-in-the-middle attacks.',
            'Weak Session ID': 'Predictable or short session IDs can be guessed by attackers, allowing unauthorized access without credentials.',
            'No Session Timeout': 'Sessions that never expire allow attackers unlimited time to exploit stolen session tokens, even after users logout.',
            'Missing SameSite': 'Without SameSite attribute, cookies are sent in cross-site requests, enabling CSRF attacks that perform actions as authenticated users.',
        }
        return explanations.get(vuln_type, 'This session vulnerability could allow attackers to hijack user sessions and gain unauthorized access.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Session Fixation': '"Regenerate session ID after login: session_regenerate_id(true); or session.regenerate() in Express. Always create new session on authentication state change"',
            'Missing HttpOnly Flag': '"Set HttpOnly flag on all session cookies: Set-Cookie: sessionid=xxx; HttpOnly; Secure; SameSite=Strict. In Express: cookie: {httpOnly: true}"',
            'Missing Secure Flag': '"Enable Secure flag for HTTPS-only transmission: Set-Cookie: sessionid=xxx; Secure; HttpOnly. In Django: SESSION_COOKIE_SECURE = True"',
            'Weak Session ID': '"Use cryptographically secure random session IDs: secrets.token_urlsafe(32) in Python or crypto.randomBytes(32) in Node.js. Minimum 128-bit entropy"',
            'No Session Timeout': '"Implement session timeout: absolute timeout 24 hours, idle timeout 30 minutes. Check last_activity timestamp on each request and invalidate if expired"',
            'Missing SameSite': '"Add SameSite attribute: Set-Cookie: sessionid=xxx; SameSite=Strict; HttpOnly; Secure. Use Strict for same-site only, Lax for cross-site GET"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by implementing secure session management best practices"')
    ...
    def _check_session_compliance(self, severity):
