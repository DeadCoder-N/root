import { X, Lock, AlertTriangle, Download, Loader2 } from 'lucide-react';
...
export default function OAuthModal({ isOpen, onClose }: OAuthModalProps) {
  ...
  const handleAnalyze = async () => {
    if (!url.trim()) {
      setError('Please enter an OAuth endpoint URL');
      return;
    }
    ...
    try {
      const response = await fetch('http://localhost:5007/api/analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ url: url.trim(), flow_type: flowType }),
      });
      ...
      setError(err.message || 'Failed to analyze OAuth security');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5007/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `oauth-security-report-${Date.now()}.pdf`;
      ...
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-green-500 to-teal-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">OAuth Security Tester</h2>
              <p className="text-white/80 text-sm">Test OAuth 2.0 implementation security</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">OAuth Endpoint URL *</label>
              <input
                type="url"
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                placeholder="https://example.com/oauth/authorize"
                className="w-full px-4 py-2 bg-secondary border border-white/10 rounded-lg focus:outline-none focus:border-green-500/50"
                disabled={loading}
              />
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">OAuth Flow Type</label>
              <select
                value={flowType}
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-green-500 to-teal-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
                ...
                <div className="space-y-3">
                  ...
                  {results.vulnerabilities.map((vuln: any, index: number) => (
                    <div key={index} className="p-4 bg-secondary border border-white/10 rounded-lg space-y-2">
                      ...
                      {vuln.fix_prompt && (
                        <div className="mt-2 p-3 bg-primary border border-green-500/30 rounded">
                          <div className="text-xs text-green-500 font-medium mb-1">Fix Recommendation:</div>
          ...
          {!results && !loading && !error && (
            <div className="text-center py-12 space-y-4">
              <Lock className="w-16 h-16 text-text-muted mx-auto opacity-50" />
              <div>
                <h3 className="text-lg font-semibold mb-2">How to Use</h3>
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the OAuth authorization endpoint URL</p>
                  <p>• Select the OAuth flow type being used</p>
                  <p>• Tests state parameter, redirect URI, PKCE, code injection, token leakage</p>
                  <p>• Get comprehensive security report with remediation steps</p>
                </div>
              </div>
            </div>
          )}
