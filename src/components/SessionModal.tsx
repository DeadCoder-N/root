import { X, Activity, AlertTriangle, Download, Loader2 } from 'lucide-react';
...
export default function SessionModal({ isOpen, onClose }: SessionModalProps) {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5008/api/analyze', {
        method: 'POST',
      ...
      setError(err.message || 'Failed to analyze session management');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5008/api/download', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      a.href = url;
      a.download = `session-analysis-report-${Date.now()}.pdf`;
      ...
      <div className="bg-primary border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="bg-gradient-to-r from-yellow-500 to-amber-500 p-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">Session Analyzer</h2>
              <p className="text-white/80 text-sm">Analyze session management security</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-gradient-to-r from-yellow-500 to-amber-500 text-white py-3 rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          {!results && !loading && !error && (
            <div className="text-center py-12 space-y-4">
              <Activity className="w-16 h-16 text-text-muted mx-auto opacity-50" />
              <div>
                <h3 className="text-lg font-semibold mb-2">How to Use</h3>
                <div className="text-sm text-text-muted space-y-2 max-w-md mx-auto text-left">
                  <p>• Enter the target website URL</p>
                  <p>• Tests session fixation, CSRF protection, cookie security</p>
                  <p>• Checks session timeout and regeneration</p>
