"""XXE Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"xxe_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="XXE Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("XXE SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive XML External Entity Injection Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['XML Security', self._check_xxe_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected XML endpoints and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'XXE File Disclosure': 'Attackers can read any file on server (passwords, source code, /etc/passwd) by injecting external entity references in XML input.',
            'XXE SSRF': 'XML parsers can be exploited to make requests to internal systems, bypassing firewalls and accessing internal APIs, databases, or cloud metadata.',
            'XXE Denial of Service': 'Billion Laughs attack uses nested entities to consume all server memory and CPU, crashing the application and causing downtime.',
            'Blind XXE': 'Even without direct output, attackers can exfiltrate data to external servers or trigger out-of-band interactions to confirm vulnerability.',
            'XXE Parameter Entity': 'Parameter entities in DTD allow attackers to bypass input filters and read files or perform SSRF attacks through XML processing.',
            'Unsafe XML Parser': 'XML parsers with external entity processing enabled are vulnerable to XXE attacks by default, requiring explicit security configuration.',
        }
        return explanations.get(vuln_type, 'This XXE vulnerability allows attackers to read files, perform SSRF, or cause denial of service through XML processing.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'XXE File Disclosure': '"Disable external entities in XML parser. Python: from defusedxml import ElementTree; tree = ElementTree.parse(file). PHP: libxml_disable_entity_loader(true). Java: factory.setFeature(XMLConstants.FEATURE_SECURE_PROCESSING, true)"',
            'XXE SSRF': '"Disable DTD and external entities. Python lxml: parser = etree.XMLParser(resolve_entities=False, no_network=True). .NET: XmlReaderSettings.DtdProcessing = DtdProcessing.Prohibit"',
            'XXE Denial of Service': '"Limit entity expansion. Java: factory.setFeature(XMLConstants.FEATURE_SECURE_PROCESSING, true); factory.setProperty(XMLConstants.ACCESS_EXTERNAL_DTD, \'\'); factory.setProperty(XMLConstants.ACCESS_EXTERNAL_SCHEMA, \'\')"',
            'Blind XXE': '"Use defusedxml library (Python): from defusedxml.ElementTree import parse; tree = parse(file). Disable all external processing: parser.resolveEntities = False"',
            'XXE Parameter Entity': '"Disable parameter entities. PHP: libxml_disable_entity_loader(true); $dom->loadXML($xml, LIBXML_NOENT | LIBXML_DTDLOAD | LIBXML_DTDATTR). Use JSON instead of XML when possible"',
            'Unsafe XML Parser': '"Switch to safe parser. Python: use defusedxml instead of xml.etree. Java: DocumentBuilderFactory.setFeature(\'http://apache.org/xml/features/disallow-doctype-decl\', true)"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by disabling external entity processing and DTD in XML parser configuration"')
    ...
    def _check_xxe_compliance(self, severity):
