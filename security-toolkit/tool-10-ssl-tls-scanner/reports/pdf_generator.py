"""SSL/TLS Security Report Generator - Professional Grade"""
...
class UniversalReportGenerator:
    def __init__(self, results: dict):
        ...
        self.filename = f"ssl_security_{datetime.now().strftime('%d_%b_%Y_%I_%M_%p')}.pdf"
    ...
    #     doc = SimpleDocTemplate(output_path, pagesize=letter, rightMargin=0.75*inch, leftMargin=0.75*inch, topMargin=0.75*inch, bottomMargin=0.75*inch, title="SSL/TLS Security Report")
    ...
    def _create_title_page(self):
        ...
        elements.append(Paragraph("SSL/TLS SECURITY ASSESSMENT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Comprehensive Certificate & Encryption Security Analysis", self.styles['Normal']))
    ...
    def _create_executive_summary(self):
        ...
        dashboard_data = [
            ...
            ['TLS Compliance', self._check_tls_compliance(severity), ''],
        ]
    ...
    def _create_detailed_findings(self):
        ...
        elements.append(Paragraph("Vulnerabilities grouped by type. Each group shows affected certificates/domains and a single fix command.", self.body_style))
        elements.append(Spacer(1, 0.2*inch))
    ...
    def _group_vulnerabilities(self, vulnerabilities):
        ...
        for vuln in vulnerabilities:
            ...
            groups[vuln_type]['locations'].append({'location': vuln.get('domain', vuln.get('url', 'N/A')), 'evidence': vuln.get('evidence', 'N/A')})
    ...
    def _format_vulnerability_group(self, group):
        ...
        elements.append(Paragraph(f"<b>🔒 Affected Domains ({group['count']}):</b>", self.body_style))
        for i, loc in enumerate(group['locations'][:10]):
            elements.append(Paragraph(f"  • {loc['location']}", self.body_style))
        if group['count'] > 10:
            elements.append(Paragraph(f"  ... and {group['count'] - 10} more domains", self.body_style))
        if group['fix_prompt']:
    ...
    def _get_explanation(self, vuln_type):
        explanations = {
            'Expired Certificate': 'Browsers will show security warnings and block access to your site. Users cannot trust the connection, leading to lost traffic and revenue.',
            'Self-Signed Certificate': 'Self-signed certificates are not trusted by browsers, showing security warnings. Attackers can easily perform man-in-the-middle attacks.',
            'Weak Cipher Suite': 'Weak ciphers can be broken by attackers, allowing decryption of encrypted traffic. This exposes sensitive data like passwords and credit cards.',
            'TLS 1.0/1.1 Enabled': 'Old TLS versions have known vulnerabilities (BEAST, POODLE) that allow attackers to decrypt traffic. Modern browsers are disabling support.',
            'Missing HSTS': 'Without HSTS, users can be downgraded to HTTP, allowing man-in-the-middle attacks to intercept traffic before HTTPS redirect.',
            'Certificate Mismatch': 'Domain name mismatch causes browser warnings and indicates potential man-in-the-middle attack or misconfiguration.',
        }
        return explanations.get(vuln_type, 'This SSL/TLS vulnerability compromises encryption security and exposes data to interception.')
    
    def _generate_ai_prompt(self, vuln):
        prompts = {
            'Expired Certificate': '"Renew SSL certificate immediately. Let\'s Encrypt: certbot renew. Commercial: contact CA for renewal. Automate with: certbot renew --deploy-hook \'systemctl reload nginx\'"',
            'Self-Signed Certificate': '"Get trusted certificate from Let\'s Encrypt (free): certbot --nginx -d example.com. Or purchase from DigiCert, Sectigo. Install with: certbot install --cert-path /path/to/cert"',
            'Weak Cipher Suite': '"Update cipher suite to modern secure ciphers. Nginx: ssl_ciphers \'ECDHE-ECDSA-AES128-GCM-SHA256:ECDHE-RSA-AES128-GCM-SHA256\'; Apache: SSLCipherSuite ECDHE-ECDSA-AES128-GCM-SHA256"',
            'TLS 1.0/1.1 Enabled': '"Disable old TLS versions. Nginx: ssl_protocols TLSv1.2 TLSv1.3; Apache: SSLProtocol -all +TLSv1.2 +TLSv1.3. Restart web server after change"',
            'Missing HSTS': '"Enable HSTS header. Nginx: add_header Strict-Transport-Security \'max-age=31536000; includeSubDomains; preload\'; Apache: Header always set Strict-Transport-Security \'max-age=31536000\'"',
            'Certificate Mismatch': '"Get certificate for correct domain. Let\'s Encrypt: certbot --nginx -d correct-domain.com. Ensure certificate CN/SAN matches domain name exactly"',
        }
        return prompts.get(vuln['type'], f'"Fix {vuln["type"]} by updating SSL/TLS configuration to modern security standards"')
    ...
    def _check_tls_compliance(self, severity):
